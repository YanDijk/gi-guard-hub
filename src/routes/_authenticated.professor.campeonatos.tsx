...
function Campeonatos() {
  ...
  const createT = useMutation({
    mutationFn: async (p: { name: string; city: string; event_date: string; registration_url?: string }) => {
    ...
    >
      {showForm && (
        <TournamentForm
          ...
          onSubmit={(p) => createT.mutate(p)}
          saving={createT.isPending}
        />
      )}
            ...
            return (
              <div key={t.id} className="bg-surface border border-border rounded-lg overflow-hidden flex flex-col">
                <div className="aspect-[16/9] bg-gradient-to-br from-brand/30 to-surface-2 flex items-center justify-center relative">
                  ...
                  <button
                    onClick={() => {
                      if (confirm(`Excluir "${t.name}"?`)) deleteT.mutate(t.id);
                    }}
                    className="absolute top-2 right-2 size-7 grid place-items-center bg-black/60 rounded text-muted-foreground hover:text-red-400"
                  >
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="flex gap-2 mt-auto pt-4">
                    <button
                      ...
                      className="flex-1 h-9 bg-brand text-brand-foreground rounded-md text-xs font-bold uppercase tracking-wider"
                    >
                      {expanded === t.id ? "Fechar" : "Gerenciar"}
                    </button>
                    {t.registration_url && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="h-9 px-3 border border-border rounded-md text-xs uppercase tracking-wider hover:border-brand inline-flex items-center gap-1"
                      >
                        ...
                        {ts.map((s) => {
                          ...
                          return (
                            <div key={s.id} className="flex items-center gap-2 text-xs">
                              ...
                              <button
                                onClick={() => removeSignup.mutate(s.id)}
...
function SignupPicker({
  ...
  if (students.length === 0) {
    return <div className="text-xs text-muted-foreground">Todos os alunos já foram convocados.</div>;
  }
  return (
...
function TournamentForm({
  ...
  onCancel: () => void;
  onSubmit: (p: { name: string; city: string; event_date: string; registration_url?: string }) => void;
  saving: boolean;
}) {
  const [form, setForm] = useState({ name: "", city: "", event_date: new Date().toISOString().slice(0, 10), registration_url: "" });
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        if (!form.name.trim()) return toast.error("Informe o nome");
        onSubmit({ ...form, registration_url: form.registration_url || undefined });
      }}
      className="bg-surface border border-border rounded-lg p-6 mb-6 grid grid-cols-4 gap-3"
    >
